...
public class Main {
	...
	private static void randomLetter(City city) {
		...
		if(dice == 0) letter = new SimpleLetter(sender, receiver,"Greetings M. Roos :)");
		if(dice == 1)  letter = new PromissoryNote(sender, receiver, random.nextInt(1000));
		if(dice == 2)  letter = new RegisteredLetter<Letter<?>>(sender, receiver, letter);
		if(dice == 3)  letter = new UrgentLetter<Letter<?>>(sender, receiver, letter);
		if(dice == 4)  {letter = new PromissoryNote(sender, receiver, random.nextInt(1000)); letter = new UrgentLetter<Letter<?>>(sender, receiver, letter); }
		if(dice == 5)  {letter = new PromissoryNote(sender, receiver, random.nextInt(1000)); letter = new RegisteredLetter<Letter<?>>(sender, receiver, letter); }
